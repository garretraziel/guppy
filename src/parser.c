...
static int statement(void)
{
    ...
    switch(token) {
        ...
        case IF:
            ...
            // vyraz
            try( expression() );
            ptr1 = generate(IJMPF, NULL, 0); // skok na else pri false
            // then
            check_token(THEN, ERROR_SYN_X_THEN);
            get_token();
            // prikazy
            try( statement_seq() );
            ptr2 = generate(IJMP, NULL, 0); // skok na konec
            // else
            check_token(ELSE, ERROR_SYN_X_ELSE);
            get_token();
            lab1 = generate(INOP, NULL, 0); // zacatek else
            ...
            check_token(END, ERROR_SYN_X_END);
            get_token();
            lab2 = generate(INOP, NULL, 0); // konec
            ptr2->adr = lab2;
            return 1;
        ...
        case WHILE:
            get_token();
            // label
            lab1 = generate(INOP, NULL, 0);
            // vyraz
            try( expression() );
            // skok
            ptr1 = generate(IJMPF, NULL, 0);
            // do
            ...
            generate(IJMP, lab1, 0);
            lab2 = generate(INOP, NULL, 0);
            ptr1->adr = lab2;
            get_token();
            return 1;
